...
load_dotenv()
# Only set environment variables if they are present to avoid assigning None
_serper = os.getenv("SERPER_API_KEY")
if _serper is not None:
    os.environ["SERPER_API_KEY"] = _serper
_reddit_id = os.getenv("REDDIT_CLIENT_ID")
if _reddit_id is not None:
    os.environ["REDDIT_CLIENT_ID"] = _reddit_id
_reddit_secret = os.getenv("REDDIT_CLIENT_SECRET")
if _reddit_secret is not None:
    os.environ["REDDIT_CLIENT_SECRET"] = _reddit_secret
_reddit_agent = os.getenv("REDDIT_USER_AGENT")
if _reddit_agent is not None:
    os.environ["REDDIT_USER_AGENT"] = _reddit_agent
